...
class FacebookRadar: 
    ...
    def run_interactive(self):
        ...
        # SỬA: Chỉ chạy 1 lần, không tự động lặp
        hours_back = self.settings.get("scan_hours", 24)
        if isinstance(hours_back, str):
            try:
                hours_back = int(hours_back)
            except:
                hours_back = 24
        self.run_full_scan(hours_back=hours_back)
...
def main():
    ...
    parser.add_argument('--mode', choices=['scan', 'interactive', 'monitor', 'test'],
                       default='scan',  # SỬA: Mặc định là 'scan' thay vì 'interactive'
    ...
    if args.mode == 'test':
        ...
        radar.auto_run(args.interval)  # SỬA: Gọi đúng phương thức

    elif args.mode == 'interactive':
        radar.run_interactive()  # SỬA: Gọi run_interactive thay vì auto_run
